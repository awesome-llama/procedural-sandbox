...
def optimal_binary_tree(tree, items, weights):
    ...
    for i in range(1, len(weights)):
        ...
        cost_difference = abs(total_cost_start - total_cost_end) + abs((len(weights)/2)-i)/210
    ...
    if len(weights_start) == 1:
        ...
        tree.append([])
        optimal_binary_tree(tree[0], items_start, weights_start)
    
    if len(weights_end) == 1:
        tree.append(items_end[0])
    else:
        tree.append([])
        optimal_binary_tree(tree[1], items_end, weights_end)
...
def optimal_binary_tree_from_weights(weights):
    tree = []
    optimal_binary_tree(tree, list(range(len(weights))), weights)
    return tree
