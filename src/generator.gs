...
%define COMPILE_MAIN_GENERATORS
...
# COMPILE_MAIN_GENERATORS
%if COMPILE_MAIN_GENERATORS
        ...
        if (hash < POW(stack[6]/18, 2)) {
            ...
            if (((stack[5]*721.9324) % 1) < 0.5) {
                ...
                if ((((stack[5]*396.1209) % 1) < $probability_half) and (stack[4] % 2 == 0)) {
                    ...
                    # split randomly anywhere
                    local split_position = round(stack[4] * (0.25+((stack[5]*7435.3425) % 0.5)));
...
%define MEM6 memory[instructions[iptr+6]]
%define MEM7 memory[instructions[iptr+7]]
...
proc language_run {
    ...
    until (iptr > length instructions) {
        ...
        ######
        # Following code generated by src/lang/instructions.py
        # DO NOT EDIT MANUALLY
        # Edit the source only and copy the output here.
        if (opcode < 16) {
            if (opcode < 3) {
                if (opcode < 1) {
                    # 0: set
                ...
                if (opcode < 25) {
                    if (opcode < 20) {
                        ...
                        if (opcode < 22) {
                            if (opcode < 21) {
                                # 20: min
                                if (MEM2 < MEM3) {MEM1 = MEM2;} else {MEM1 = MEM3;}iptr += 4;
                            } else {
                                # 21: max
                                if (MEM2 > MEM3) {MEM1 = MEM2;} else {MEM1 = MEM3;}iptr += 4;
                            }
                            ...
                            if (opcode < 23) {
                                # 22: call
                                add iptr+2 to call_stack;iptr = instructions[iptr+1];
                            } else {
                                if (opcode < 24) {
                                    # 23: return
                                    iptr = call_stack["last"];delete call_stack["last"];
                                } else {
                                    # 24: random
                                    MEM1 = random(MEM2, MEM3);iptr += 4;
                                }
                            }
                        }
                    }
                ...
                if (opcode < 50) {
                    if (opcode < 42) {
                        if (opcode < 38) {
                            if (opcode < 36) {
                                if (opcode < 35) {
                                    # 34: select
                                    if MEM2 {MEM1 = MEM3;} else {MEM1 = MEM4;}iptr += 5;
                                } else {
                                    # 35: lerp
                                    MEM1 = LERP(MEM2, MEM3, MEM4);iptr += 5;
                                }
                            } else {
                                if (opcode < 37) {
                                    # 36: asin
                            ...
                            if (opcode < 40) {
                                if (opcode < 39) {
                                    ...
                                    # 39: atan2
                                    MEM1 = ATAN2(MEM2, MEM3);iptr += 4;
                                }
                            } else {
                                if (opcode < 41) {
                                    # 40: mag2
                                    MEM1 = VEC2_LEN(MEM2, MEM3);iptr += 4;
                                } else {
                                    # 41: ln
                                    MEM1 = ln(MEM2);iptr += 3;
                                }
                        ...
                        if (opcode < 46) {
                            ...
                            if (opcode < 48) {
                                if (opcode < 47) {
                                    ...
                                    # 47: clear_canvas
                                    clear_canvas MEM1, MEM2, MEM3;iptr += 4;
                                }
                            } else {
                                if (opcode < 49) {
                                    # 48: add_canvas_as_template
                                    add_canvas_as_template;iptr += 1;
                                } else {
                                    # 49: reset_depositor
                                    reset_depositor;iptr += 1;
                                }
                            }
                        }
                    }
                    ...
                    if (opcode < 58) {
                        if (opcode < 54) {
                            if (opcode < 52) {
                                if (opcode < 51) {
                                    # 50: set_depositor_to_air
                                    set_depositor_to_air;iptr += 1;
                                } else {
                                    # 51: set_depositor_from_sRGB
                                    set_depositor_from_sRGB MEM1, MEM2, MEM3;iptr += 4;
                                }
                            } else {
                                if (opcode < 53) {
                                    # 52: set_depositor_from_HSV
                                    set_depositor_from_HSV MEM1, MEM2, MEM3;iptr += 4;
                                } else {
                                    # 53: set_depositor_from_voxel
                                    set_depositor_from_voxel MEM1, MEM2, MEM3;iptr += 4;
                                }
                            }
                        } else {
                            if (opcode < 56) {
                                if (opcode < 55) {
                                    # 54: set_depositor_to_template
                                    set_depositor_to_template MEM1, MEM2, MEM3, MEM4;iptr += 5;
                                } else {
                                    # 55: set_depositor_opacity
                                    depositor_voxel.opacity=MEM1;iptr += 2;
                                }
                            } else {
                                if (opcode < 57) {
                                    # 56: set_depositor_emission
                                    depositor_voxel.emission=MEM1;iptr += 2;
                                } else {
                                    # 57: set_voxel
                                    set_voxel MEM1, MEM2, MEM3;iptr += 4;
                                }
                            }
                        }
                    } else {
                        if (opcode < 62) {
                            if (opcode < 60) {
                                if (opcode < 59) {
                                    # 58: draw_ray
                                    draw_ray MEM1, MEM2, MEM3, MEM4, MEM5, MEM6, MEM7;iptr += 8;
                                } else {
                                    # 59: draw_cuboid_corner_size
                                    draw_cuboid_corner_size MEM1, MEM2, MEM3, MEM4, MEM5, MEM6;iptr += 7;
                                }
                            } else {
                                if (opcode < 61) {
                                    # 60: draw_sphere
                                    draw_sphere MEM1, MEM2, MEM3, MEM4;iptr += 5;
                                } else {
                                    # 61: draw_cylinder
                                    draw_cylinder MEM1, MEM2, MEM3, MEM4, MEM5;iptr += 6;
                                }
                            }
                        } else {
                            if (opcode < 64) {
                                ...
                                if (opcode < 65) {
                                    # 64: get_canvas_size
                                    MEM1=canvas_size_x;MEM2=canvas_size_y;MEM3=canvas_size_z;
                                } else {
                                    if (opcode < 66) {
                                        # 65: unlerp
                                        MEM1 = UNLERP(MEM2, MEM3, MEM4);iptr += 5;
                                    } else {
                                        # 66: print
...
proc random_walk_any x, y, z, start_dir, turns, steps, angle {
    ...
    repeat $turns {
        ...
        draw_ray agent_x, agent_y, $z, cos(agent_dir), sin(agent_dir), 0, dist;
...
# 3D DDA
proc draw_ray x, y, z, dx, dy, dz, r {
